...
if(!b.configurado||(b.consultas||[]).filter(c=>c.rota==='usuario').some(c=>typeof c.status!=='number'||c.status<200||c.status>=300))throw new Error('A conexão ainda não está pronta para as ações.');

if(b.consultas?.some(c=>c.rota==='grupo-tarefa'&&c.status===404))console.log('Tarefas remotas indisponíveis: grupo-tarefa respondeu 404; demais ações mantidas, sem criar grupo fictício.');
