...
export default function PlanejamentoMarketing({mapa,aoAtualizar}){
 ...
 const [form,setForm]=useState(null),[salvando,setSalvando]=useState(false),[erro,setErro]=useState(''),[filtro,setFiltro]=useState(''),[mes,setMes]=useState('');
 ...
 <label className="label mb-4">Mês do prazo<input className="input max-w-xs" type="month" value={mes} onChange={e=>setMes(e.target.value)}/></label>{mes&&<button className="btn-ghost mb-3" onClick={()=>setMes('')}>Ver todos os meses</button>}
 <div className="flex flex-wrap gap-2 mb-4">{['',...ETAPAS].map(s=><button key={s} className={filtro===s?'btn-primary':'btn-ghost'} onClick={()=>setFiltro(s)}>{s||'Todas'} · {acoes.filter(a=>(!s||a.status===s)&&(!mes||a.prazo?.startsWith(mes))).length}</button>)}</div>
 ...
 <label className="label">Situação<select className="input" value={form.status} onChange={campo('status')}>{ETAPAS.map(s=><option key={s}>{s}</option>)}</select></label>
 ...
 <div className="grid gap-3 lg:grid-cols-2">{acoes.filter(a=>(!filtro||a.status===filtro)&&(!mes||a.prazo?.startsWith(mes))).map(a=>{
 ...
 })}</div>{!acoes.filter(a=>(!filtro||a.status===filtro)&&(!mes||a.prazo?.startsWith(mes))).length&&<p className="text-sm text-slate-500 py-4">Nenhuma ação neste recorte. Cadastre uma ação com responsável e prazo para começar.</p>}
