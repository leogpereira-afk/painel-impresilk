import {getRecebiveis,getOrdensServico} from '../services/mubi.js';
...
export default function Cliente360({alvo,dados,hoje,indisponiveis=[],crm,erroCrm,aoFechar,aoOrcamento,aoVincular}){
 ...
 useEffect(()=>{let vivo=true;setFinanceiro(null);Promise.allSettled([getRecebiveis(),getOrdensServico()]).then(r=>{if(vivo)setFinanceiro({recebiveis:r[0].status==='fulfilled'?r[0].value:null,ordens:r[1].status==='fulfilled'?r[1].value:null});});return()=>{vivo=false;};},[alvo.clienteId]);
 ...
 const f=useMemo(()=>fichaCliente360(alvo,{...dados,recebiveis:financeiro?.recebiveis || [],ordens:financeiro?.ordens || []},res?.cliente,hoje),[alvo,dados,res,hoje,financeiro]);
 ...
 return <section className="cliente360 sem-impressao" aria-labelledby="cliente360-titulo">
  ...
  <div><h3>Financeiro e serviços</h3>{!financeiro?.recebiveis?<p>{financeiro?'Recebíveis indisponíveis para esta consulta.':'Consultando recebíveis…'}</p>:!f.vinculoFinanceiro?<p>Saldo não associado: falta um vínculo por cadastro ou documento.</p>:<><p>Saldo em aberto: <b>{moeda(f.saldo)}</b></p><p>Vencido: <b>{moeda(f.saldoVencido)}</b> · {f.vencidos.length} títulos</p></>}{!financeiro?.ordens?<p>{financeiro?'Ordens de serviço indisponíveis para esta consulta.':'Consultando serviços…'}</p>:!f.vinculoOrdens?<p>Serviços ainda sem vínculo confirmado com este cadastro.</p>:<p>{f.ordens.length} ordens na base carregada{f.ordens[0]?.data&&` · última em ${dataCurta(f.ordens[0].data)}`}</p>}<small>Cobertura limitada às fontes carregadas. Nenhuma quitação é presumida pela ausência de títulos.</small></div></div>
