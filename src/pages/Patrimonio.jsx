...
function CapaBem({bem, quantidade, revisao, pequena = false}) {
  ...
  // Na lista: a mesma foto (buscada só quando a linha aparece), em miniatura.
  if (pequena) return <div ref={ref} className="pat-thumb">{url && !falhou ? <img src={url} alt="" loading="lazy" onError={()=>setFalhou(true)}/> : <Camera size={16} aria-hidden="true"/>}</div>;
...
export default function Patrimonio() {
  ...
  return <div className="patrimonio-page">
    ...
    {aba==='inventario'&&<>
      ...
      </article>)}</div>:<div className="pat-table-wrap"><table className="pat-table"><thead><tr><th>Seleção</th><th>Bem / etiqueta</th><th>Setor</th><th>Situação</th><th>Valor de aquisição</th><th>Ficha</th></tr></thead><tbody>{nestaPagina.map(b=><tr key={b.id}><td><input type="checkbox" aria-label={`Selecionar ${b.codigo||b.nomeGenerico}`} checked={selecionados.includes(b.id)} onChange={()=>selecionar(b.id)}/></td><td><div className="pat-table-bem"><button className="pat-thumb-btn" onClick={()=>verDetalhe(b)} aria-label={`Abrir fotos e ficha de ${b.codigo||b.nomeGenerico}`}><CapaBem bem={b} quantidade={fotos===null?undefined:(fotos[b.id]||0)} revisao={revisaoFotos} pequena/></button><div><button className="pat-table-name" onClick={()=>verDetalhe(b)}>{b.nomeGenerico}</button><small>{b.codigo||'Sem etiqueta'} · {b.descricaoTecnica||'Sem descrição'}</small></div></div></td><td>{nomeSetor(b,vm.setores)}</td><td><Situacao bem={b}/></td><td>{b.valor>0?moedaCheia(b.valor):'A informar'}</td><td><button className="btn-ghost" aria-label={`Ver ficha de ${b.codigo||b.nomeGenerico}`} onClick={()=>verDetalhe(b)}><ArrowUpRight size={18}/></button></td></tr>)}</tbody></table></div>}
